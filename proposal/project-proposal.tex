...
\smartdiagramx[priority descriptive diagram]{
  part of speech analysis: e.~g.~extracting verbs from cooking instructions, stemming? lemmatization?, removing stop words, converting abbreviations into long versions (e.~g.~tbsps tablespoons), data cleaning: removing duplicate recipes; removing outliers, e.~g.~very short recipes without meaningful instructions or ingredients like \emph{How to prepare frozen pizza} or very long recipes which contain sub-recipes), Recipes dataset(s)
}
